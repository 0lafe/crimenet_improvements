...
function CIM:save_current_lobby(room_id)
    self._lobby_context = {
        room_id = room_id,
...
function CIM:lobby_context()
    return self._lobby_context
end

function CIM:snapshot_lobby(handler)
...
function CIM:explain_join_fail()
    local c = self._lobby_context
    ...
    local queued = CIM:format_queued()
    if queued then
